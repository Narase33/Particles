        ...
        void GlxContext::createSurface(GlxContext* shared, unsigned int width, unsigned int height, unsigned int bitsPerPixel) {
            ...
            if (shared && (sfglx_ext_SGIX_pbuffer == sfglx_LOAD_SUCCEEDED)) {
                ...
                if (hasCreatePbuffer) {
                    ...
                    if (config) {
                        int attributes[] = {GLX_PBUFFER_WIDTH, static_cast<int>(width), GLX_PBUFFER_HEIGHT, static_cast<int>(height), 0, 0};
            ...
            m_window = XCreateWindow(m_display, RootWindow(m_display, screen), 0, 0, width, height, 0, DefaultDepth(m_display, screen), InputOutput, visualInfo.visual, CWColormap, &attributes);
